dfGather_iCAPQ <- function(df,massTrace){
  ...
  for(i in 1:length(massTrace)) {
    ...
    dataSingleTrace <- df %>% select("Index","Labels",contains(massTrace[i]));
    ...
    dataSortedSingleTrace$Wiederholung <- as.integer(gsub(paste0(massTrace[i],"."),"",  # entfernt die Bezeichnung der MasseSpur
