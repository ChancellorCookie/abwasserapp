my.tQuant.Chromatogramm.Plot <- function(myList){
  ...
  SortedChromatogramm <- myList$Chromatography$Chromatogramm$SortedChromatogramm
  ...
  for (i in 1:nrow(myList$Header)) {
    ...
    plotData <- plotData[order(plotData %>% select(contains("time"))),] %>% `names<-`(.,c("Index","Labels","Counts","Time"))
    
    # Aus Rohdaten Extraktion der Peak-Markierungen
    PeakStart <- unlist(myList$Chromatography$PeakStart$Data[i,Analytes])
    PeakEnd <- unlist(myList$Chromatography$PeakEnd$Data[i,Analytes])
    PeakMax <- unlist(myList$Chromatography$Retention$Data[i,Analytes])
    PeakArea <- unlist(myList$Chromatography$PeakArea$Data[i,Analytes])
    PeakHight <- unlist(myList$Chromatography$PeakHeight$Data[i,Analytes])
    ...
    g <- ggplot(plotData,aes(Time, Counts)) +
      ...
      labs(y = "Intensität (CPS)", # Achsenbeschriftungen, 
           x = "Time (s)",
           title = myList$Input.Parameter$QCMethod,
    ...
    Plot.List[[paste0(myList$Header$Index[i],"_",myList$Header$Labels[i])]] <- g
  }
