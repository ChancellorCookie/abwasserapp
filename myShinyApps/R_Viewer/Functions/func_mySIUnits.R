mySIUnits <- function(chr_Unit="mg"){
  ...
  for (i in 1:length(chr_Unit)) {
    ...
    if (is.na(str_Unit)) {
      output[i,"SI_Numerator"]     <- NA
      output[i,"SI_Denominator"]   <- NA
      output[i,"Factor"]           <- NA
      output[i,"not_SI_Unit"]      <- NA
      output[i,"call_Unit"]        <- NA
      output[i,"call_Numerator"]   <- NA
      output[i,"call_Denominator"] <- NA
      next
    } else if (any(str_detect(def.Non.SI()$Unit,str_Unit))){
      output[i,"SI_Numerator"]     <- NA
      output[i,"SI_Denominator"]   <- NA
      output[i,"Factor"]           <- def.Non.SI()[str_detect(def.Non.SI()$Unit,str_Unit),"Factor"]
      output[i,"not_SI_Unit"]      <- str_Unit
...
myUnitFactors <- function(df_SI_prefix = def.SI.Prefix(),df_Units_Strings){
