myDMA80.Smpl <- function(df, Smpl.filter = "^M.+?[[:digit:]]/[[:digit:]].+?"){
  ...
  temp.mean <- Maske %>%  group_by(SampleNames.Simple,Remark,Unit.2) %>% summarise(Means = mean(Concentration))
  # Berechnung der Stabw
  temp.Stabw <-  Maske %>%  group_by(SampleNames.Simple,Remark,Unit.2) %>% summarise(SD = sd(Concentration))
