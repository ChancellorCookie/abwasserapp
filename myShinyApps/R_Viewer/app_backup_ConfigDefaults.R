...
# ===== [ANLEITUNG] Erstellen eines neuen Benutzers =====
# ______________________________________________
# Benutzer des ZE1 Generators können von Benutzern mit Admin-Rechten über die Web-UI (Tab-Reiter Administration) verwaltet werden.
# Alternativ kann das Erstellen eines neuen Benutzers über die folgende Funktion in der Console von R erfolgen. 
# 
# new.user(Vorname = "Michail",Nachname = "Dronov",group = "user",password = "test")
#
# Als Ausgabe erhält man ein FALSE, wenn dieser Benutzer bereits registriert ist. 
# Username (nicht die UserID) sowie Passwort lassen sich mit hilfe der folgenden Funktionen überschreiben
#
# new.username(old.username = "m_dron01",new.username = "MightyMike69")
# Diese Funktion gibt ein FALSE zurück, wenn der neue Benutzername bereits vergeben ist
#
# new.pw(username = "m_dron01",newPW = "MightyMike69")
# Diese Funktion gibt ein FALSE zurück, wenn der username nicht existiert
#
# Das Passwort des Admins (Standard ist "admin") kann ebenfalls geändert werden. 
#
# Die UserID des Admins darf nicht geändert werden!
#
# Die Benutzer Daten sind in der R_Viewer.usr Datei (kann mit einem Standard Text-Editor geöffnet werden) zu finden.
# Die Passwörter werden nur als sog. SaltedHash-Wert in der R_Viewer.pw Datei gespeichert. Dieser Wert kann auch
# manuell zu jeder Zeichenkette berechnet werden. Berechnen kann man ihn über den folgenden Funktionaufruf in der Console.
#
# saltedHash(pw = "admin")
# 
# Der Salt-Wert innerhalb dieser Funktion sollte nicht verändert oder verraten werden, da ansonsten der PW Schutz nicht mehr gewährleistet ist, 
# und die R-Viewer.pw Datei mit neuen SaltedHash-Werten manuell initialisiert werden muss.
# ______________________________________________
...
# Items.QC <- c()
# Items.Samples <- c()
# df.InputParameter <- data.frame()
df.PipettenListe <- data.frame()
df.ini.PipettenList <- data.frame("Pipetten_ID"=c("1231","1240","1241","1246","771"),stringsAsFactors = F)
# numSigDigits <- 4
...
sidebar <- dashboardSidebar(
  # sidebarUserPanel(name = "LogIn",
  #                  subtitle = a(href = "#", icon("circle", class = "text-success"), "Online"),
  #                  # Image file should be in www/ subdir
  #                  image = NULL
  # ),
  sidebarMenu(id = "iniSideBarInputs",
              menuItemOutput("menuOut")
  )#,collapsed = TRUE
)
...
server = function(input, output,session) {
  ...
  # options(DT.options = list(pageLength = 100, language = list(search = 'Suche:')))
  ...
  dynamicResultItems <- eventReactive(vals$performRenderSidebar,{
    ...
    sidebarMenu(id = "tabs",
                ...
                if(as.logical(unique(read.defaults() %>% # Get the Chromatography tag in defaults.csv
    ...
    dynamic_SidebarTabItems <- c(
      ...
      if(!is.null(lists$myList[["QC"]])) lapply(1:length(names(lists$myList[["QC"]])), function(i){
        tabItem(tabName = paste0("qc",i),
                box(title = "Plot",collapsible = T,width = 6,plotOutput(paste0("plot.qc",i))),
                box(title = "Data",collapsible = T,width = 6,DTOutput(paste0("tbl.qc",i)))
        )
      }),
  ...
  #   }
  #   lists$myList$Header %>% select(-StartTime) #%>% hot_col("factor_allow", allowInvalid = TRUE)
  ...
  observe({
    ...
    output$dl.xlsx <- downloadHandler(
      ...
      content = function(file){
        # browser()
        if(input$dropDown.System == "eQuant"){
          writeData <- list(
        ...
        } else if(input$dropDown.System == "tQuant"){
            writeData <- list(
              "Header"= lists$myList$Header,
              "Final.Conc" = lists$myList$Concentration$Conc.Final,
    ...
    output$dl.pdf <- downloadHandler(
      ...
      content = function(file) {
        ...
        rmdFile <- file.path(tempdir(),unique(read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(Report.File)) %>% pull())
        file.copy(unique(read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(Report.File)) %>% pull(),rmdFile,overwrite = TRUE)
      ...
      lapply(1:length(lists$myList$Calibration$Info$Analytes), function(i){
        ...
        output[[paste0("tbl.CaliOLS.Param",i)]] <- renderDT({
          ...
          if(CalMeth == "Kalibriergraden-Methode"){
            ...
            Parameter <- c("b0","b1","sb0","sb1","R","m","n","Alpha","ta","xNG","xEG","xBG")
          } else {
  ...
  render.QC <- function(){
    ...
    if(any(!is.na(lists$myList$QC))){
      lapply(1:length(read.defaults() %>% filter(SAA.Menue %in% lists$myList$Input.Parameter$SAA.Selected) %>% select(QC.Name) %>% pull()), function(i){
        output[[paste0("tbl.qc",i)]] <- renderDT({
          df <- myDFsignif(lists$myList$QC[[{read.defaults() %>% 
              filter(SAA.Menue %in% lists$myList$Input.Parameter$SAA.Selected) %>% 
              select(QC.Name) %>% pull()}[i]]][["QC.Table"]],3)
          df$StartTime %<>% as.character()
          df},rownames = FALSE)
        output[[paste0("plot.qc",i)]] <- renderPlot({lists$myList$QC[[{read.defaults() %>% 
            filter(SAA.Menue %in% lists$myList$Input.Parameter$SAA.Selected) %>% 
    ...
    box(title = h2("Initial Inputs"),width = 12,
        fluidRow(
          box(title = "User Inputs",width = 3,
              ...
              selectInput(inputId = "dropDown.System",label = "Measurement-Method",choices = unique(read.defaults()[["System"]])),
  ...
  output$tab.Report <- renderUI({
    ...
    fluidPage(
      fluidRow(
        ...
        if(!is.null(lists$myList) & as.logical(unique(read.defaults() %>% # Reads the default value file
  ...
  observeEvent(input$btn.calc.solids,{
    ...
    # browser()
    #df.c_Total = Solid.Total.Concentration(lists$myList$Solids)
    #df.c_rel = Solid.Total.Relative(df.c_Total)
    ...
    if (is.null(input$num.SigDigit)) {
      SigDigit <- unique(read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(Significant.Digits))  %>% pull()
    } else {
  ...
  # Hier soll ein Tab entstehen, dass die Azeige und Modifikation gespeicherter QC Regelkarten ermöglicht. 
  # Zudem soll hier das Anlegen neuer QC Methoden ermöglicht werden, 
  # d.h., hier sollen neuen Einträge in die defaults.csv möglich sein.
  # _________________________________________________
  # [ÜBERLEGUNG] eigenständige App für diese Aufgabe
  # _________________________________________________
  output$defaults.Config <- renderUI({
    if (vals$AdminRights) {
      box(title = "QC",width = 9,
          fluidRow(
            actionButton(inputId = "btn.Add.QC",label = "Add Control Chart"),
            actionButton(inputId = "btn.Add.ControlChart",label = "Add Control Chart"),
            actionButton(inputId = "btn.Remove.ControlChart",label = "Remove Control Chart"),
            actionButton(inputId = "btn.Comment.ControlChart",label = "Comment"),
            actionButton(inputId = "btn.Add.Element",label = "Add Element"),
            actionButton(inputId = "btn.Add.Element",label = "Add Element")
          )
  ...
  output$tab.Administration <- renderUI({
    if (vals$AdminRights) {
      fluidRow(
        box(title = "User Conntrols",width = 6,
  ...
  output$BG.Selection <- renderUI({
    ...
    if(as.logical(unique(read.defaults() %>% # Get the Chromatography tag in defaults.csv
      ...
      selectInput(inputId = "dropDown.BGMethod",label = "LoQ Method",choices = c("Signal/Rausch","Signal/Rausch (vor Juni 2020)" ,"Kalibriergraden-Methode"))
    }else{
  ...
  output$SAA.Selection <- renderUI({
    ...
    selectInput(inputId = "dropDown.SAA.Menu",
                ...
                choices = unique(read.defaults() %>% filter(System %in% input$dropDown.System) %>% select(SAA.Menue)) %>% pull())
  })
  output$SAA.Admin.SigDigit <- renderUI({
    req(input$dropDown.SAA.Menu)
    numericInput(inputId = "num.SigDigit",
                 label = "Significant.Digits",
                 value = unique(read.defaults() %>% # Reads the default value file
                                  filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% # filter for selected QC Method
  ...
  output$SAA.Admin.BG.SAA <- renderUI({
    ...
    checkboxInput(inputId = "chk.BG.SAA",
                  label = "Report BG SAA",
                  value = as.logical(unique(read.defaults() %>% # Reads the default value file
                                              filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% # filter for selected QC Method
                                              select(BG.SAA))))
  })
  ...
  output$QC.Definition <- renderUI({
    ...
    #browser()
    QCSoll <- read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% filter(QC.Kind %in% "QC") %>% select(QC.Name,SAA,QC.Norm,Unit.Norm)
    if (nrow(QCSoll)>0){
  ...
  observeEvent(input$dropDown.System,{
    ...
    output$UI.IntCorr <- renderUI({
      if (!any(is.na(read.defaults() %>% filter(System %in% input$dropDown.System) %>% select(UseIntCorr)))) {
        checkboxInput(inputId = "chk.IntCorr",label = "Internal Correction",
                      value = all(read.defaults() %>% filter(System %in% input$dropDown.System) %>% select(UseIntCorr)))
      }
    })
    
    # Checkbox for use of averaged values for computation ----
    output$UI.Means <- renderUI({
      if (!any(is.na(read.defaults() %>% filter(System %in% input$dropDown.System) %>% select(UseMeans)))) {
        checkboxInput(inputId = "chk.Means",label = "Use means",
                      value = all(read.defaults() %>% filter(System %in% input$dropDown.System) %>% select(UseMeans)))
      }
    })
    
    # Checkbox for use of outlier test for computation ----
    output$UI.Outliers <- renderUI({
      if (!any(is.na(read.defaults() %>% filter(System %in% input$dropDown.System) %>% select(outlier)))) {
        checkboxInput(inputId = "chk.Outliers",label = "Exclude outliers",
                      value = all(read.defaults() %>% filter(System %in% input$dropDown.System) %>% select(outlier)))
      }
    })
  })
  ...
  output$Trace.Assign <- renderUI({
    req(!is.null(lists$myList))
    if(as.logical(unique(read.defaults() %>% filter(SAA.Menue %in% lists$myList$Input.Parameter$SAA.Selected) %>% select(Analyt.User.Selection)))){ 
      Assign <- data.frame("Element" = lists$myList$Calibration$Info$Elements,
                           "Analyte" = lists$myList$Calibration$Info$Analytes,
                           stringsAsFactors = F)
      
      
      # Element <- read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(Element,Analyt)
      lapply(1:length(unique(Assign$Element)), function(i) {
        ...
        selected <- unique(read.defaults() %>% filter(SAA.Menue %in% lists$myList$Input.Parameter$SAA.Selected) %>% select(Element,Analyt))[["Analyt"]] # get defaults.csv defined traces
  ...
  # output$ReportMasses <- renderUI({
  #   req(input$dropDown.SAA.Menu)
  #   browser()
  #   textInput(inputId = "txt.Mass",label = "Mass",value = read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(Masse) %>% pull()) 
  # })
  ...
  observeEvent(input$FileInput,{
    ...
    vals$FileToLoad <- normalizePath(inFile,winslash = "\\",mustWork = T)
    ...
    if(file_ext(vals$FileToLoad) == "csv" || file_ext(vals$FileToLoad) == "txt" ){
      ...
      if(is.null(input$dropDown.BGMethod)){
        ...
        lists$myParams$BGMethod <- unique(read.defaults() %>% filter(System %in% input$dropDown.System) %>% select(BGMethod)) %>% pull()
      ...
      }else if(input$dropDown.BGMethod == "Signal/Rausch (vor Juni 2020)"){
      ...
      if(is.null(input$num.SigDigit)){
        # Load Defaults for "User" usage
        lists$myParams$Significant.Digits <- unique(read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(Significant.Digits))  %>% pull()
      }else{
      ...
      if(is.null(input$chk.BG.SAA)){
        # Load Defaults for "User" usage
        lists$myParams$BG.SAA <- as.logical(unique(read.defaults() %>% # Reads the default value file
                                                     filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% # filter for selected QC Method
                                                     select(BG.SAA)))
      }else{
      ...
      if(is.null(input$num.Alpha)){
        # Load Defaults for "User" usage
        lists$myParams$Alpha <- unique(read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(Alpha)) %>% pull()
      }else{
        lists$myParams$Alpha <- input$num.Alpha
      }
      
      # if(is.null(input$txt.Mass)){
      #   # Load Defaults for "User" usage
      #   lists$myParams$Masse <- unique(read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(Masse) %>% pull() %>% as.character())
      ...
      # Load Defaults for "User" usage
      lists$myParams$Signal.Name <- unique(read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(Signal.Name) %>% pull() %>% as.character())
      lists$myParams$Masse <- unique(read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(Masse) %>% pull() %>% as.character())
      # if not as.character -> Error in dplyr contains() function!
      
      if(is.null(input$chk.Means)){
        # Load Defaults for "User" usage
        lists$myParams$UseMeans <- unique(read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(UseMeans)) %>% pull()
      }else{
      ...
      if(is.null(input$chk.Outliers)){
        # Load Defaults for "User" usage
        lists$myParams$outlier <- unique(read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(outlier)) %>% pull()
      }else{
      ...
      if(is.null(input$chk.IntCorr)){
        # Load Defaults for "User" usage
        lists$myParams$UseIntCorr <- unique(read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(UseIntCorr)) %>% pull()
      }else{
      ...
      if (as.logical(unique(read.defaults() %>% filter(System %in% input$dropDown.System) %>% select(Cali.User.Selection)))) { # If user input is neccessary
        if(is.null(dfs$cali.files.selected)){ # If no selection performed by user in calibration tab (e.g. no Admin Rights)
          ...
          lists$myParams[["Calibration"]][["filename"]] <- unique(read.defaults() %>% # Reads the default value file
                                                                    filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% # filter for selected QC Method
      ...
      }else if (is.na(unique(read.defaults() %>% filter(System %in% input$dropDown.System) %>% select(Cali.Default.File)))){ # If the calibration data is included in the Raw-File (e.g. eQuant)
        # Set all values NA
      ...
      }else if (input$dropDown.System == "eQuant") {
        # browser() # only for debugging
        # myParams <- isolate(lists$myParams)
    ...
    # Ist überflüssig und kann entfern werden.
    if(file_ext(vals$FileToLoad) == "RData" ){
      
      if (length(lists$myList) == 0) {
        # Load the File
        showModal(modalDialog("Loading...Please wait!", footer=NULL))
        myEnv <- new.env()
        name <- load(vals$FileToLoad,envir = myEnv)
        lists$myList <- myEnv[[name]]
        removeModal()
      }
    }
    
    # Trigger variable for re-render the sidebar
    vals$performRenderSidebar <- vals$performRenderSidebar + 1
  ...
  observeEvent(vals$perform.QC,{
    ...
    QCData <- read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(SAA,QC.Kind,QC.Name,Filename)
    QCSoll <- read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% filter(QC.Kind %in% "QC") %>% select(QC.Name,SAA,QC.Norm,Unit.Norm)
    ...
    if(!is.null(input[[paste0("QCSoll.",QCSoll$SAA[1],".",QCSoll$QC.Name[1])]])){ # Check if input is given
      ...
      for (i in 1:nrow(QCData)) {
        lists$myList[["Input.Parameter"]][["QC"]][[QCData$QC.Name[i]]] <- QCData[i,"Filename"]
      }
    }else {
    ...
    lists$myList[["QC"]] <- myQC.Defaults.csv(results.list = myList.Converter(lists$myList), # Converts the measurement results in a uniform list
                                              ...
                                              NonDefaultAnalyt.vec = vals$NonDefaultAnalyt.vec,path.QCtoWrite = path.QCtoWrite) # In the UI defined QCSoll values
  ...
  observeEvent(input$chk.BG.SAA,{
    ...
    if(input$chk.BG.SAA){
      ...
      BGs <- read.defaults() %>% # Reads the default value file
        ...
        select(LimitBG)
      names(BGs) <- read.defaults() %>% # Reads the default value file
        filter(SAA.Menue %in% lists$myList$Input.Parameter$SAA.Selected) %>% # filter for selected QC Method
        filter(QC.Kind %in% "VBW") %>% # filter for selected QC.Kind (LimitBG is only stored for VBW)
        select(Analyt)
    }else{
  ...
  # The preselection is defined in a "default"-Parameter .csv file and loaded by --> read.defaults() 
  cali.files.pre_selection <- function (cali.files.all) { # Function to determine the pre_selection of QC_Dropdown Menue ----
    ...
    selCali <-  unique(read.defaults() %>% filter(SAA.Menue %in% input$dropDown.SAA.Menu) %>% select(Cali.Default.File))
  ...
  output$box.CaliDef <- renderUI({  ## Build Calibration Elements in UI ----
    ...
    if (as.logical(unique(read.defaults() %>% filter(System %in% input$dropDown.System) %>% select(Cali.User.Selection)))){
  ...
  #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  write.Calibrtaion.file <- function(cali.files.selected,path){
    # Function to write calibration files in the new format
    # Attributes are written in square brackets.
    
    # 
    # [names] Al, Pt, Cis-Platin, Hg, ...
    # [units] mg/L, ng/L, µg/L, ng(abs), ...
    # [row.names] S0, S1, S2, ...
    # 0, 0, 0, 0, ...
    # 0.1, 0.5, 5, 0.5, ...
    # 0.5, 1, 10, 1, ...
  ...
  convert.Calibrtaion.file <- function(path){
    cali.df <- read.csv(file = path,
                        header = F,
                        na.strings = "",
                        stringsAsFactors = F)
    
    write.Calibrtaion.file(cali.df,path)
  }
  
  read.Calibration.file <- function(path){
    
    if (READ.FAIL) {
      convert.Calibrtaion.file(path)
    }
  ...
  observeEvent(input$select.Cali,{ ## Observed functions triggert by changed selected calibration file.
    ...
    vals$loaded.Cali.File.Path <- paste0(path.Calibration,"/",input$select.Cali)
  ...
  observeEvent(input$num.Rows,{ # Dynamic change of Table Row-Dimension ----
    ...
    output$txt_ConfimeSave <- renderText({""})
  ...
  observeEvent(input$btn_save, {
    # TODO replace Path----
    fileName <- paste0(path.Calibration,"/",input$txt_Save)
  ...
  observeEvent(input$btn_delCali,{
    ...
    observeEvent(input$btn_delete,{
      ...
      file.remove(paste0(path.Calibration,"/",input$select.Cali))
      # Display information text for override
      output$txt_ConfimeSave <- renderText({
  ...
  observeEvent(input$btn.reset.PW,ignoreInit = TRUE,{
    ...
    observeEvent(input$btn.change.password,ignoreInit = TRUE, {
      ...
      # output$tbl.Users <- renderDataTable({
      #   df.ini.UserTable <<- read.csv(file = "R_Viewer.usr",stringsAsFactors = F)
      #   datatable(df.ini.UserTable,options=list(pageLength = 100),selection="single")
      # })
      vals$reload_adminDF <- vals$reload_adminDF +1
    })
  })
