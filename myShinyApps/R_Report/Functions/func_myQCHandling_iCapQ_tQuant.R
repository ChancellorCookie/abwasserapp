myQCHandling_iCapQ_tQuant <- function(myList = myList,
                         QCFilter = "^M.+?QC VBW",
                         ColumnFilter = "Cis",
                         Handling = "VBW",
                         FileToWrite = paste0(normalizePath(getwd()),"\\QC\\Regelkarten\\tQuant_RegelKartenTest_VBW.csv"),
                         TargetValue = NA,
                         KillOld = FALSE){
  ...
  if (Handling == "VBW") { 
    ...
    QC <- myList$Header %>% select(Index,Labels,StartTime) %>% 
      merge(.,myList$tQuant$Chromatography$PeakArea$Data %>% select(Index,Labels,contains(ColumnFilter)),by = c("Index","Labels")) %>%
      merge(.,myList$tQuant$Evaluation$Concentration %>% select(Index,Labels,contains(ColumnFilter)),by = c("Index","Labels")) %>%
      filter(str_detect(Labels,QCFilter)) %>%
      select(-Index)
    
    names(QC) <- c("Labels","StartTime","Area","Concentration")
    ...
    Conc.QC <- myList$tQuant$Evaluation$Concentration %>% 
      filter(str_detect(Labels,QCFilter)) %>% 
