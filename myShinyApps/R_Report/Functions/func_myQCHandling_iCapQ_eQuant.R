myQCHandling_iCapQ_eQuant <- function(myList = myList,
  ...
  if (Handling == "VBW") { 
    ...
    QC <- myList$Header %>% select(Index,Labels,StartTime) %>% 
      merge(.,myList$eQuant$Raw$Raw.Average$Data %>% select(Index,Labels,contains(ColumnFilter)),by = c("Index","Labels")) %>%
      merge(.,myList$eQuant$Evaluation$Concentration %>% select(Index,Labels,contains(ColumnFilter)),by = c("Index","Labels")) %>%
    ...
    Conc.QC <- myList$eQuant$Evaluation$Concentration %>% 
      filter(str_detect(Labels,QCFilter)) %>% 
