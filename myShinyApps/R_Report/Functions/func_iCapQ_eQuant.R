iCapQ_eQuant <- function(myPreList,myParams){
  ...
  if(length(myPreList.Survey.Raw) != 0){
    myPreList.Survey.Raw$Info <-  myRawDataExtraction(myPreList.Survey.Raw)
  }else{
  ...
  if(length(myPreList.Survey.ExtCal) != 0){
    myPreList.Survey.ExtCal$Info <-  myRawDataExtraction(myPreList.Survey.ExtCal)
  }else{
  ...
  myPreList.Raw$Info <-  myRawDataExtraction(myPreList.Raw)
  ...
  myPreList.ExtCal$Info <-  myRawDataExtraction(myPreList.ExtCal)
  ##################################################
  ...
  myPreList.ExtCal[["Which.Analytes"]] <- ExtCal.which.Analytes # Check
  
  myPreList.IntStd <- list("Raw.Average"= IntStd.Raw.Average, # Check
                        "Average.Factors" = IntStd.Factors.Average, # Check
                        "Raw.Intensity"= IntStd.Raw.Intensity, # Check
                        "Intensity.Factors" = IntStd.Factors.Intensity, # Check
                        "Which.Standard" = IntStd.which.Standards, # Check
                        "Assignment" = IntStd.Assignment) # Check
  ...
  ###### Output #####
  outList <- list("Input.Parameter" = myParams) # Check
  
  outList[["Raw.Data"]] <- myPreList$Raw.Data
  
  outList[["Prepared"]] <- myPreList$AllData
  
  outList[["Header"]] <- header[order(header$Index),] # Check
  
  outList[["eQuant"]] <- list("Raw" = myPreList.Raw, # Check
                             "ExtCal" = myPreList.ExtCal, # Check
                             "IntStd" = myPreList.IntStd,
                             "Std.Concentration" = Standard.Concentration, # Check
                             "Std.Raw.Average" = Standard.Raw.Average, # Check
                             "Std.Raw.Intensity" = Standard.Raw.Intensity) # Check
  
  
  
  outList[["Survey"]] <- list("Raw" = myPreList.Survey.Raw, # Check
                              "ExtCal" = myPreList.Survey.ExtCal) #Check
